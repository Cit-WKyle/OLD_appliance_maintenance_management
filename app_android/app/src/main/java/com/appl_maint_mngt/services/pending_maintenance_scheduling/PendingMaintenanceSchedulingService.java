...
import com.appl_maint_mngt.event_bus.common.ApplicationEventBus;
...
public class PendingMaintenanceSchedulingService implements IPendingMaintenanceSchedulingService {
        ...
        httpClient.post(context, IPendingMaintenanceSchedulingResources.ADD_RESOURCE, entity, IWebConstants.CONTENT_TYPE_JSON, new JsonHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                ...
                if(apiResponse.getData()) {
                    ApplicationEventBus.getInstance().sendEvent(IPendingMaintenanceSchedulingEvents.CREATED_EVENT);
                } else {
    ...
    public void accept(Long id, final IErrorCallback errorCallback) {
        httpClient.post(IPendingMaintenanceSchedulingResources.BASE_RESOURCE + id + IPendingMaintenanceSchedulingResources.ACCEPT_PATH, new RequestParams(), new JsonHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                logger.d("onSuccess post: %s", response.toString());
                ApplicationEventBus.getInstance().sendEvent(IPendingMaintenanceSchedulingEvents.ACCEPTED_EVENT);
            }
    ...
    public void decline(Long id, final IErrorCallback errorCallback) {
        httpClient.post(IPendingMaintenanceSchedulingResources.BASE_RESOURCE + id + IPendingMaintenanceSchedulingResources.DECLINE_PATH, new RequestParams(), new JsonHttpResponseHandler() {
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                logger.d("onSuccess post: %s", response.toString());
                ApplicationEventBus.getInstance().sendEvent(IPendingMaintenanceSchedulingEvents.DECLINED_EVENT);
            }
